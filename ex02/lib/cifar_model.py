...
class ConvModel(nn.Module):
    def __init__(self, input_channels: int, num_filters: int, verbose: bool = False):
        ...
        self.conv1 = nn.Conv2d(input_channels, num_filters, kernel_size=3,
                               padding=1, stride=2)
        self.bn1 = nn.BatchNorm2d(num_filters)
        self.relu1 = nn.ReLU()

        self.conv2 = nn.Conv2d(num_filters, 2*num_filters, kernel_size=3,
                               padding=1, stride=1)
        self.bn2 = nn.BatchNorm2d(num_filters*2)
        self.relu2 = nn.ReLU()

        self.avgpool = nn.AvgPool2d(kernel_size=16, stride=16)
        self.ln = nn.Linear(2*num_filters, 10)
        # END TODO ###################
